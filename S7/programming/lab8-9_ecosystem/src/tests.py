...
from utils import euclidean_distance, check_hexacolor
...
class MonkeyModelTestCase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.expected_bmi = self.expected_weight / (self.expected_size**2)
    ...
    def test_constructor(self):
        ...
        # Checking that the monkey exists
        ex = self.ex_monkey
        assert ex
        
        # Checking that he has the expected attributes
        assert ex.species == self.expected_species
        assert ex.weight == self.expected_weight
        assert ex.size == self.expected_size
        assert ex.fur_color == self.expected_fur_color

    def test_str(self):
        """testing the representation matches expections"""
        assert __str__(self.ex_monkey) == self.expected_str
        assert __repr__(self.ex_monkey) == self.expected_repr

    def test_check_fur(self):
        """testing checkhexacolor is correctly applied"""
        assert check_hexacolor(self.ex_monkey.fur_color)

    def test_bmi(self):
        """testing Body Mass Index computations"""
        assert self.ex_monkey.compute_bmi() == self.expected_bmi
...
class MonkeyCSVTestCase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.sup_cols = "monkey", "color_int", "bmi"
